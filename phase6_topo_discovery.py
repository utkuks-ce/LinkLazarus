...
class LLDPPathController(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        # LED agent ayarları (controller tarafı)
        self.LED_UDP_PORT = 9999  # led_agent.py'nin dinlediği UDP port

        self.led_agents = {
            963354559889: "192.168.1.11",  # 000000e04c680591 (pi1)
            963354559857: "192.168.1.2",   # 000000e04c680571 (pi2)
            963354560655: "192.168.1.3",   # 000000e04c68088f (pi3)
            963354560406: "192.168.1.4",   # 000000e04c680796 (pi4)
            963354560387: "192.168.1.5",   # 000000e04c680783 (pi5)
        }
    ...
    def _seed_local_mac(self, dp):
        try:
            ofp = dp.ofproto
            p = dp.ports.get(ofp.OFPP_LOCAL)
            if p and getattr(p, "hw_addr", None):
                mac = p.hw_addr.lower()
                self.local_host_mac[dp.id] = mac
                self.logger.info(f"[LOCAL-HOST] seeded from OFPP_LOCAL dpid={dp.id} mac={mac}")
        except Exception as e:
            self.logger.debug(f"[LOCAL-HOST] seed fail dpid={dp.id}: {e}")
    ...
    def _dump_state(self):
        # Topoloji
        try:
            sws = sorted(self.topology.nodes())
            print(f"[DUMP] switches={sws}", flush=True)
            es = []
            for u, v, data in self.topology.edges(data=True):
                es.append((u, v, data.get('port')))
            es.sort()
            print(f"[DUMP] edges={es}", flush=True)
        except Exception as e:
            print(f"[DUMP] topo error: {e}", flush=True)

        # Datapath’ler
        try:
            dps = sorted(list(self.datapaths.keys()))
            print(f"[DUMP] datapaths={dps}", flush=True)
        except Exception as e:
            print(f"[DUMP] dps error: {e}", flush=True)

        # Edge/uplink port setleri
        try:
            for sw in sorted(self.topology.nodes()):
                edges = sorted(list(self.edge_ports(sw)))
                upls  = sorted([p for (s,p) in self.link_ports if s==sw])
                print(f"[DUMP] DPID={sw} edge_ports={edges} uplinks={upls}", flush=True)
        except Exception as e:
            print(f"[DUMP] ports error: {e}", flush=True)

        # Host tabloları
        try:
            print(f"[DUMP] local_host_mac={self.local_host_mac}", flush=True)
            print(f"[DUMP] mac_to_loc={self.mac_to_loc}", flush=True)
            print(f"[DUMP] ip_to_mac={self.ip_to_mac}", flush=True)
        except Exception as e:
            print(f"[DUMP] learn error: {e}", flush=True)




    @staticmethod
    def _parse_dpid(x: str) -> int:
        x = x.strip().lower().replace(":", "").replace("-", "")
        x = x.strip().lower()
        if x.startswith("0x"):
            return int(x, 16)
        # saf hex (örn. 000000e04c680591) ise:
        if all(c in "0123456789abcdef" for c in x) and 1 <= len(x) <= 16:
            return int(x, 16)
        # değilse decimal dene
        return int(x, 10)
    

    def _notify_led_agents(self, emerg_on: bool, dpids=None):
        import socket
        msg = b"EMERG ON" if emerg_on else b"EMERG OFF"
        peers = []
        if dpids:
            for d in dpids:
                ip = self.led_agents.get(d)
                if ip:
                    peers.append(ip)
        else:
            peers = list(set(self.led_agents.values()))
        s = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        for ip in peers:
            try:
                s.sendto(msg, (ip, self.LED_UDP_PORT))
                self.logger.info(f"[EMERG][LED] notify {ip}:{self.LED_UDP_PORT} -> {msg.decode()}")
            except Exception as e:
                self.logger.error(f"[EMERG][LED] notify fail {ip}: {e}")
        s.close()
    ...
    def _emerg_cmd_listener(self):
        ...
        sock.settimeout(1.0)
        # Hem logger hem print (print flush=True → logging config’e takılmayalım)
        self.logger.warning("[EMERG] Listening UDP on 0.0.0.0:%d", self.EMERG_UDP_PORT)
        print(f"[EMERG][DBG] listener bound 0.0.0.0:{self.EMERG_UDP_PORT}", flush=True)
        ...
        while True:
            try:
                ...
                print(f"[EMERG][DBG] listener exception: {e}", flush=True)
                traceback.print_exc()
                break

            raw = data.decode(errors="ignore").strip()
            up  = raw.upper()
            print(f"[EMERG][DBG] recv from {addr[0]}:{addr[1]} raw='{raw}' up='{up}'", flush=True)

            parts = up.split()
            if not parts or parts[0] != "EMERG":
                self.logger.error(f"[EMERG] Unknown command: '{raw}' from {addr[0]}")
                print(f"[EMERG][DBG] unknown '{raw}'", flush=True)
                continue

            # ---- DEBUG komutları ----
            if len(parts) == 2 and parts[1] == "PING":
                self.logger.info(f"[EMERG] PING from {addr[0]}")
                print(f"[EMERG][DBG] PING ok from {addr[0]}", flush=True)
                continue

            if len(parts) == 2 and parts[1] == "DUMP":
                print("[EMERG][DBG] DUMP requested", flush=True)
                self._dump_state()
                continue
            ...
            if len(parts) >= 2 and parts[1] == "OFF":
                ...
                print(f"[EMERG][DBG] OFF requested", flush=True)
                try:
                    self._emerg_off()
                except Exception as e:
                    self.logger.error(f"[EMERG] OFF error: {e}")
                    traceback.print_exc()
                continue
            ...
            if len(parts) == 4 and parts[1] == "ON":
                try:
                    s_raw, d_raw = parts[2], parts[3]
                    # decimal ya da hex string gelebilir
                    def norm(x):
                        try:
                            return int(x, 0)  # "0x..." ise hex, yoksa decimal
                        except Exception:
                            return int(x)     # fallback
                    s = norm(s_raw); d = norm(d_raw)
                    self.logger.warning(f"[EMERG] ON requested: src_dpid={s}, dst_dpid={d} from {addr[0]}")
                    print(f"[EMERG][DBG] ON parsed s={s} d={d}", flush=True)
                    self._emerg_on(s, d)
                    print(f"[EMERG][DBG] ON done s={s} d={d}", flush=True)
                except Exception as e:
                    self.logger.error(f"[EMERG] Bad ON command '{raw}': {e}")
                    print(f"[EMERG][DBG] ON error: {e}", flush=True)
                    traceback.print_exc()
                continue

            # Diğerleri
            self.logger.error(f"[EMERG] Unknown subcommand '{raw}' from {addr[0]}")
            print(f"[EMERG][DBG] unknown subcmd '{raw}'", flush=True)
    ...
    def _emerg_on(self, src_dpid, dst_dpid):
        ...
        try:
            self._notify_led_agents(True, [src_dpid, dst_dpid])
        except Exception as e:
            self.logger.error(f"[EMERG][LED] notify error: {e}")
    ...
    def apply_emergency_policy(self):
        ...
        # önce tüm datapath'lerden eski EMERG kurallarını temizle
        for dp in list(self.datapaths.values()):
            self.remove_flows_by_cookie(dp, self.EMERG_COOKIE)
        ...
        for i in range(len(path) - 1):
            ...
            # A->B
            self.add_l2_flow(dp, macA, macB, outp, priority=100, ip_proto=17,
                 idle_timeout=0, hard_timeout=0, cookie=self.EMERG_COOKIE)
        ...
        for sw, dp in self.datapaths.items():
            self.add_drop_flow_pair(dp, macA, macB, priority=110, idle_timeout=0, hard_timeout=0, cookie=self.EMERG_COOKIE)
            self.add_drop_flow_pair(dp, macB, macA, priority=110, idle_timeout=0, hard_timeout=0, cookie=self.EMERG_COOKIE)
    ...
    def switch_enter_handler(self, ev):
        ...
        self._seed_local_mac(ev.switch.dp)
    ...
    def link_delete_handler(self, ev):
        ...
        # Topolojiyi komple yeniden hesapla (edge/uplink setleri + ARP ingress kuralları güncellensin)
        self.update_topology()

        # Emergency açıksa yeni path'e göre akışları tekrar uygula
        if self.emerg_active:
            self.apply_emergency_policy()
